...
class LandDetails : AppCompatActivity() {
    ...
    private lateinit var tvLandForSaleOr: TextView
    private lateinit var tvLandOwnerOr: TextView
    private lateinit var tvLandDetailStatus: TextView
        ...
        ivProductDetail.setItemClickListener(object : ItemClickListener {
            override fun onItemSelected(position: Int) {
                if (product.pictures[position] != "") {
                    val intent = Intent(this@LandDetails, FillScreenImageView::class.java)
    ...
    private fun setProductValuesToViews(product: ProductsModelParcelable) {
        ...
        if (product.pictures.size > 1) {
            ...
            imageList.add(SlideModel(R.drawable.category_land))
        }

        tvProductDetailsID.text = product.uid
        tvTitleProductDetails.text = "Location: ${product.location}"
        tvPriceProductDetails.text = "Br ${NumberFormat.getInstance(Locale.US).format(product.price)}"
        tvDescription.text = product.description
        tvLocationDetails.text = product.location
        tvSizeDetails.text = "${product.size} Square Meter"
        tvLandForSaleOr.text = product.purpose
        tvLandOwnerOr.text = product.seller
        ...
        if (product.sold) {
            tvLandDetailStatus.visibility = View.VISIBLE
            tvLandDetailStatus.bringToFront()
        } else {
            tvLandDetailStatus.visibility = View.GONE
        }
    }
    ...
    private fun setupControlViews() {
        ...
        tvLandForSaleOr = findViewById(R.id.tvLandForSaleOr)
        tvLandOwnerOr = findViewById(R.id.tvLandOwnerOr)
        tvLandDetailStatus = findViewById(R.id.tvLandDetailStatus)
    }

    private fun setupCustomProgressDialog() {
        val alertView = View.inflate(this@LandDetails, R.layout.custom_progress_bar,null)

        val alertBuilder = AlertDialog.Builder(this@LandDetails)
